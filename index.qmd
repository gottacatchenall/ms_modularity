...
qualitatively very different partitions $\vec{b}$. This was first reported in  
@Good2010PerMod, who also show that $Q_max$ is highly dependent on the number of 
clusters and the size of the network, and conclude---"[the] modules identified 
...
circumvent the issue of resolution limits from modularity maximization. Modern 
methods include the _microcanonical version_ of both the DC-SBM and NSBM 
[@Peixoto2017NonBay]. Here _microcanonical_ is terminology being adopted from 
statistical mechanics, and in practice means these models are defined for networks
with a fixed degree sequence. For a thorough review of block modeling, see @Lee2019RevSto.
