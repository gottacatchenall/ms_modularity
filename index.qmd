...
These interactions form networks that enable the persistence of
...
describes how "how well" nodes of a network can be grouped into _modules_, first
introduced in @Newman2004FinEva. Modularity has been widely adopted as a metric
of interest in ecological networks, and in principle the grouping of
...
networks is Modularity Maximization (MM), which has fundemental flaws for
robustly identifying modules in networks. 

This method originated during the mid-2000s [@Newman2004FinEva] and was
popularized through the efficeincy of the Clauset-Newman-Moore (CNM)
algorithm [@Clauset2004FinCom]. Six years later after its proposal,
@Good2010PerMod showed that in practical contexts modularity maximization is
flawed for all but idealized networks. More recently, @Peixoto2021DesVs more
thoroughly explores this issue, showing how MM can massively overfit and find
highly modular partitions ($Q \approx 0.5$) in random networks. 

Still, MM is widely used in ecological network papers. In a brief literature
survey, we found MM methods overwhelmingly prevelent in the analysis of
ecological networks.  Here we cover what modularity maximization is, and why it
doesn't work for identifying modules/groups in networks. We suggest methods for
community detection based on nested-stochastic-block-models (NSBMs) for
identifying modules in ecological networks.
...
- `bipartite` R package uses Beckett (2016), version of MM specifically for
bipartite networks. 
...
## Conclusion --- principled community detection is great, modularity is meaningless. 
